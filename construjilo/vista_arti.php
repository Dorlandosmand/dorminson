...
$limit = 10; // Artículos por página
$page = isset($_GET['page']) ? (int)$_GET['page'] : 1;
$offset = ($page - 1) * $limit;

// Consulta a la vista con límite y desplazamiento
$sql = "SELECT * FROM vista_articulos LIMIT $limit OFFSET $offset";
$result = $conn->query($sql);

// Mostrar los resultados en una tabla
if ($result->num_rows > 0) {
    echo "<table>";
    echo "<tr><th>ID Artículo</th><th>Nombre</th><th>Precio</th></tr>";

    while($row = $result->fetch_assoc()) {
        echo "<tr><td>" . $row["id_articulo"] . "</td><td>" . $row["Nombre"] . "</td><td>" . $row["Precio"] . "</td></tr>";
    }
    echo "</table>";
} else {
    echo "<p style='text-align: center;'>No hay artículos disponibles</p>";
}

// Contar el total de artículos para la paginación
$sql_total = "SELECT COUNT(*) AS total FROM vista_articulos";
