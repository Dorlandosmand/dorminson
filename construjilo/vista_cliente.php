...
$sql = "SELECT * FROM vista_clientes LIMIT $limit OFFSET $offset";
$result = $conn->query($sql);

// Mostrar los resultados en una tabla
if ($result->num_rows > 0) {
    echo "<table>";
    echo "<tr><th>ID Cliente</th><th>Nombre</th><th>Segundo Nombre</th><th>Apellido Paterno</th><th>Apellido Materno</th><th>Teléfono</th><th>Domicilio</th><th>Sexo</th></tr>";

    while($row = $result->fetch_assoc()) {
        echo "<tr><td>" . $row["id_cliente"] . "</td><td>" . $row["Nombre"] . "</td><td>" . $row["Seg_nom"] . "</td><td>" . $row["Ap_pat"] . "</td><td>" . $row["Ap_mat"] . "</td><td>" . $row["Telefono"] . "</td><td>" . $row["Domicilio"] . "</td><td>" . $row["Sexo"] . "</td></tr>";
    }
    echo "</table>";
} else {
    echo "<p style='text-align: center;'>No hay clientes disponibles</p>";
}

// Contar el total de clientes para la paginación
$sql_total = "SELECT COUNT(*) AS total FROM vista_clientes";
