...
$limit = 10; // Notas por página
$page = isset($_GET['page']) ? (int)$_GET['page'] : 1;
$offset = ($page - 1) * $limit;

// Consulta a la vista con límite y desplazamiento
$sql = "SELECT * FROM vista_notas LIMIT $limit OFFSET $offset";
$result = $conn->query($sql);

// Mostrar los resultados en una tabla
if ($result->num_rows > 0) {
    echo "<table>";
    echo "<tr><th>ID Nota</th><th>ID Cliente</th><th>ID Artículo</th><th>ID Envío</th><th>ID Estatus</th><th>Cantidad</th><th>IVA</th><th>Importe</th></tr>";

    while($row = $result->fetch_assoc()) {
        echo "<tr><td>" . $row["id_nota"] . "</td><td>" . $row["cliente_id"] . "</td><td>" . $row["articulo_id"] . "</td><td>" . $row["envio_id"] . "</td><td>" . $row["estatus_id"] . "</td><td>" . $row["cantidad"] . "</td><td>" . $row["IVA"] . "</td><td>" . $row["Importe"] . "</td></tr>";
    }
    echo "</table>";
} else {
    echo "<p style='text-align: center;'>No hay notas disponibles</p>";
}

// Contar el total de notas para la paginación
$sql_total = "SELECT COUNT(*) AS total FROM vista_notas";
